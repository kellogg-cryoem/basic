...
% Hint: get(hObject,'Value') returns toggle state of foc
load_listbox(handles); 
update_msg(handles); 

% --- Executes on button press in efar.
function efar_Callback(hObject, eventdata, handles)
% hObject    handle to efar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of efar
load_listbox(handles); 
update_msg(handles); 
...
% Hint: get(hObject,'Value') returns toggle state of enr
load_listbox(handles); 
update_msg(handles); 
...
function update_filefilters(handles)
 if(get(handles.leginon1,'Value') )
   ...
   set(handles.foc_filter,'String','*foc.mrc');
   set(handles.efar_filter,'String','*efar.mrc'); 
